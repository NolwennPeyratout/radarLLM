...
to include the additional dataset Nu-Caption from https://huggingface.co/datasets/Senqiao/LiDAR-LLM-Nu-Caption
...
import lavis.utils.geom
import lavis.utils.py
import lavis.utils.vox as vox
...
class NuscData(torch.utils.data.Dataset):
    def __init__(self, nusc: NuScenes, nusc_maps: dict, is_train: bool, data_aug_conf: dict,
                 ...
                 use_shallow_metadata: bool = True, use_pre_scaled_imgs: bool = False, custom_dataroot: str = None,
                 ...
                 get_val_rain: bool = False, get_val_night: bool = False, print_details: bool = False, dataset_name: str = 'LiDAR-LLM-Nu-Caption'):
        self.nusc = nusc
        ...
        if self.dataset_name == 'LiDAR-LLM-Nu-Caption':
            if self.is_train:
                self.QApath = os.path.join(self.dataroot, 'LiDAR-LLM-Nu-Caption', 'train.json')
            else:
                self.QApath = os.path.join(self.dataroot, 'LiDAR-LLM-Nu-Caption', 'val.json')

        elif self.dataset_name == 'nuScenesQA':
            if self.is_train:
                self.QApath = os.path.join(self.dataroot, 'nuScenesQA', 'train.json')
            else:
                self.QApath = os.path.join(self.dataroot, 'nuScenesQA', 'val.json')

        else:
            raise ValueError(f"Unsupported dataset name: {self.dataset_name}")
        
        #Maybe need to add a break if file not found.
        with open(self.QApath, 'r') as f:
            self.QAdata = json.load(f)
        ...
        self.vox_util = vox.Vox_util(
            Z, Y, X,
            scene_centroid=torch.from_numpy(self.centroid).unsqueeze(0).float().cuda(),
            bounds=self.bounds,
            assert_cube=False)
        ...
        self.logger_names = list(logger_initialized.keys())
        self.logger_name = self.logger_names[0] if self.logger_names else 'mmcv'

        if self.print_details:
            # print('ixes', self.ixes.shape)
            print_log("indices: %s", self.indices.shape, logger=self.logger_name)
            print_log(self, logger=self.logger_name)
    ...
    def get_image_data(self, rec, cams):
        ...
        for cam in cams:
            ...
            intrin = lavis.utils.geom.scale_intrinsics(intrin.unsqueeze(0), sx, sy).squeeze(0)

            fx, fy, x0, y0 = lavis.utils.geom.split_intrinsics(intrin.unsqueeze(0))

            new_x0 = x0 - crop[0]
            new_y0 = y0 - crop[1]

            pix_T_cam = lavis.utils.geom.merge_intrinsics(fx, fy, new_x0, new_y0)
    ...
    def get_seg_bev(self, lrtlist_cam, vislist):
        ...
        corners_cam = lavis.utils.geom.get_xyzlist_from_lrtlist(lrtlist_cam)  # B, N, 8, 3
    ...
    def get_lrtlist(self, rec):
        ...
        for tok in rec['anns']:
            ...
            lrt = lavis.utils.py.merge_lrt(l, lavis.utils.py.merge_rt(r, t))
    ...
    def getQA(self, sample):
        # get question and answer pairs for the current scene
        questions, answers, ids = [], [], []
        sample_token = sample['token']

        for item in self.QAdata:
            if item['sample_token'] == sample_token:
...
class VizData(NuscData):
    ...
    def get_single_item_by_qa_index(self, indexcam, index_qa):
        cams = self.choose_cams()
        item = self.get_single_item(indexcam, cams )
        ...
        #get the question, answer for the position of index_qa
        if index_qa not in all_qa_ids:
            raise KeyError(f"qa id {index_qa} not found for sample index {indexcam}")
        qa_index = all_qa_ids.index(index_qa)
        question = all_questions[qa_index]
        answer = all_answers[qa_index]
        qa_id = all_qa_ids[qa_index]
        
        
        if self.radar_encoder_type is not None:
            return imgs, rots, trans, intrins, voxel_input_feature_buffer, voxel_coordinate_buffer, number_of_occupied_voxels, question, answer, qa_id
        else:
                return imgs, rots, trans, intrins, question, answer, qa_id
    ...
    def get_single_item(self, index, cams, refcam_id: int = None):
        ...
        questions, answers, qa_ids = self.getQA(rec)
        ...
        V = 700 * self.nsweeps  # if nsweeps = 3 -> V=2100
        if radar_data.shape[0] > V:
            print_log("radar_data: %s", radar_data.shape, logger=self.logger_name)
            print_log("max pts: %d", V, logger=self.logger_name)
        ...
        if self.radar_encoder_type is not None:
            ...
            velo_T_cams = lavis.utils.geom.merge_rt(rots, trans)  # (6,4,4)
            cams_T_velo = lavis.utils.geom.safe_inverse(velo_T_cams)  # (6,4,4)
            ...
            if self.radar_encoder_type == "voxel_net":
                ...
                return imgs, rots, trans, intrins, questions, answers, qa_ids, voxel_input_feature_buffer, voxel_coordinate_buffer, number_of_occupied_voxels
        else:
                return imgs, rots, trans, intrins, questions, answers, qa_ids
    ...
    def __getitem__(self, index):
        ...
        for index_t in samples:
            if self.radar_encoder_type == "voxel_net":
                # voxelnet
                imgs, rots, trans, intrins,questions, answers, qa_ids, \
            ...
            all_qa_ids.append(qa_ids)
        all_imgs = torch.stack(all_imgs)
        all_rots = torch.stack(all_rots)
        all_trans = torch.stack(all_trans)
        all_intrins = torch.stack(all_intrins)
        # Keep QA as python lists; they can contain raw text strings.
        # Stacking with torch only works on tensors, not variable-length text.
        all_questions = list(all_questions)
        all_answers = list(all_answers)
        all_qa_ids = list(all_qa_ids)
        ...
        if self.radar_encoder_type == "voxel_net":
            ...
            return all_imgs, all_rots, all_trans, all_intrins, \
                 all_voxel_input_feature_buffer, all_voxel_coordinate_buffer, all_number_of_occupied_voxels, all_questions, all_answers, all_qa_ids
        else:
            return all_imgs, all_rots, all_trans, all_intrins, \
                all_questions, all_answers, all_qa_ids
