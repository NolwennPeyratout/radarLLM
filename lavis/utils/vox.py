...
class Vox_util(object):
    ...
    def Ref2Mem(self, xyz, Z, Y, X, assert_cube=False):
        ...
        xyz = lavis.utils.geom.apply_4x4(mem_T_ref, xyz)
        return xyz
    ...
    def Mem2Ref(self, xyz_mem, Z, Y, X, assert_cube=False):
        ...
        xyz_ref = lavis.utils.geom.apply_4x4(ref_T_mem, xyz_mem)
        return xyz_ref
    ...
    def get_mem_T_ref(self, B, Z, Y, X, assert_cube=False, device='cuda'):
        ...
        center_T_ref = lavis.utils.geom.eye_4x4(B, device=device)
        ...
        mem_T_center = lavis.utils.geom.eye_4x4(B, device=device)
        mem_T_center[:, 0, 0] = 1. / vox_size_X
        mem_T_center[:, 1, 1] = 1. / vox_size_Y
        mem_T_center[:, 2, 2] = 1. / vox_size_Z
        mem_T_ref = lavis.utils.basic.matmul2(mem_T_center, center_T_ref)
    ...
    def get_voxelnet_dense_feature_voxels(self, xyz, feat, Z, Y, X, clean_eps=0, xyz_zero=None,
        ...
        for b in range(B):
            ...
            for i, voxel_idx in enumerate(voxel_indices[b, :, :]):
                ...
                #If there is a voxel already for the point add the point to this existing voxel
                #Check to have less than 10 points per voxel and to not add the dummy coords to the voxels
                if voxel_idx in voxel_dict and not \
                        all([v_idx == 0 for v_idx in voxel_idx]) and not \
                        torch.eq(query_point, dummy_coords[b]).all().item() and voxel_k_dict[voxel_idx][1] < 9:
                    # add to existing points
                ...
                #Create a new voxel for the point
                elif not all([v_idx == 0 for v_idx in voxel_idx]) and not \
                        torch.eq(query_point, dummy_coords[b]).all().item():
                    # add new key
                    voxel_dict[voxel_idx] = [i]
    ...
    def unproject_image_to_mem(self, rgb_camB, pixB_T_camA, camB_T_camA, Z, Y, X, assert_cube=False, xyz_camA=None):
        ...
        if xyz_camA is None:
            xyz_memA = lavis.utils.basic.gridcloud3d(B, Z, Y, X, norm=False, device=pixB_T_camA.device)
            xyz_camA = self.Mem2Ref(xyz_memA, Z, Y, X, assert_cube=assert_cube)

        xyz_camB = lavis.utils.geom.apply_4x4(camB_T_camA, xyz_camA)
        z = xyz_camB[:, :, 2]

        xyz_pixB = lavis.utils.geom.apply_4x4(pixB_T_camA, xyz_camA)
        ...
        if (0):
            ...
            for b in list(range(B)):
                values[b] = lavis.utils.samp.bilinear_sample_single(rgb_camB[b], x_pixB[b], y_pixB[b])
        else:
            # native pytorch version
            y_pixB, x_pixB = lavis.utils.basic.normalize_grid2d(y, x, H, W)
    ...
    def warp_tiled_to_mem(self, rgb_tileB, pixB_T_camA, camB_T_camA, Z, Y, X, DMIN, DMAX, assert_cube=False):
        ...
        z_tileB, y_pixB, x_pixB = lavis.utils.basic.normalize_grid3d(z_tileB, y, x, D, H, W)
    ...
    def xyz2circles(self, xyz, radius, Z, Y, X, soft=True, already_mem=True, also_offset=False, grid=None):
        ...
        if grid is None:
            grid_z, grid_y, grid_x = lavis.utils.basic.meshgrid3d(B, Z, Y, X, stack=False, norm=False, device=xyz.device)
    ...
    def xyz2circles_bev(self, xyz, radius, Z, Y, X, already_mem=True, also_offset=False):
        ...
        grid_z, grid_x = lavis.utils.basic.meshgrid2d(B, Z, X, stack=False, norm=False, device=xyz.device)
        # note the default stack is on -1
