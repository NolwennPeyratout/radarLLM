# reference: https://github.com/aharley/simple_bev/blob/main/utils/improc.py
import cv2
import numpy as np
import torch
...
def oned2inferno(d, norm=True):
    ...
    if norm:
        d = lavis.utils.basic.normalize(d)
...
def draw_frame_id_on_vis(vis, frame_id, scale=0.5, left=5, top=20):
    ...
    frame_str = lavis.utils.basic.strnum(frame_id)
...
def draw_metrics_on_vis(vis, obj_iou=None, map_iou=None, scale=0.5, left=5, top=20, right=100):
    # rgb = vis.detach().cpu().numpy()[0]
    rgb = vis
    # rgb = np.transpose(rgb, [1, 2, 0]) # put channels last  # rgb
    rgb = cv2.cvtColor(rgb, cv2.COLOR_RGB2BGR)
    color = (0, 0, 0)

    if obj_iou is not None:
        obj_iou_str = lavis.utils.basic.strnum(obj_iou)
        obj_iou_str = "o: " + obj_iou_str
    ...
    if map_iou is not None:
        map_iou_str = lavis.utils.basic.strnum(map_iou)
        map_iou_str = "m: " + map_iou_str

        cv2.putText(
            rgb,
            map_iou_str,
            (right, top),  # from left, from top
            cv2.FONT_HERSHEY_SIMPLEX,
            scale,  # font scale (float)
            color,
            1)  # font thickness (int)
