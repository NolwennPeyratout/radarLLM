...
class Blip2T5Instruct(Blip2Base):
    ...
    def forward(self, samples):
        ...
        #Processing images with the ViT with ln_vison and visual encoder
        image = samples["image"]
        with self.maybe_autocast():
            image_embeds = self.ln_vision(self.visual_encoder(image))
        #Take the output of the visual encoder and pass it through a layer norm, then create attention masks for the image embeddings
        image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(image.device)

        #QFORMER FOR TEXT AND IMAGE
        #Expand the query tokens to match the batch size of the image embeddings
        ...
        if self.qformer_text_input:
            ...
            #Create attention masks for the query tokens and concatenate them with the attention masks for the text input
            query_atts = torch.ones(query_tokens.size()[:-1], dtype=torch.long).to(image.device)
            Qformer_atts = torch.cat([query_atts,text_Qformer.attention_mask],dim=1)

            #Apply the QFormer to the query tokens and the image embeddings, using the attention masks for both the query tokens and the image embeddings
            #The image is used for the cross attention, while the text input is used for the self attention in the QFormer
            ...
            #Apply the QFormer to the query tokens and the image embeddings ONLY ( not text ), using the attention masks for both the query tokens and the image embeddings
            query_output = self.Qformer.bert(
                query_embeds=query_tokens,
                encoder_hidden_states=image_embeds,
                encoder_attention_mask=image_atts,
        ...
        #In the query_output.last_hidden_state I have the query token and then the text input (if qformer_text_input is True)
        #We use only the query token part which fuses image and text infos.
        #The t5_proj will project the QFormer output to the T5 hidden size, and then we create attention masks for the T5 input
        ...
        #Processing of the input and output text with the tokenizer of the t5 model
        with self.maybe_autocast(dtype=torch.bfloat16):
            ...
            #Build the attention masks for the T5 encoder
            encoder_atts = torch.cat([atts_t5, input_tokens.attention_mask], dim=1)

            #Build the text target, target = -100 are ignored so we set the paddding token to -100.
            targets = output_tokens.input_ids.masked_fill(
                output_tokens.input_ids == self.t5_tokenizer.pad_token_id, -100
            )

            #Create the input embedding from the concatenated QFormer output and the text input embedding.
            ...
            #get the output from t5 model and calculate the loss with the text output tokens as target
