...
class FCN(nn.Module):
    ...
    def forward(self, x):
        ...
        B, k, t, _ = x.shape
        # Ensure linear and bn layers are on the same device as input
...
class VFE(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # get the maximum value across the max_points_per_voxel dimension (dim=2) for each feature channel, resulting in a feature vector that represents the most prominent features within the voxel.
        # (batch_size x num_voxels x 1 x self.half_out_channels)
        laf = torch.max(pwf, dim=2, keepdim=True)[0]

        # Repeat and concatenate with calculated features of each point = for each point in the voxel we have the same locally aggregated feature vector
        ...
        # Concatenate the point-wise features and the locally aggregated features along the feature dimension results in a new feature vecor = 2x out_channels
        pwcf = torch.cat((pwf, laf), dim=-1)

        # apply mask, the value is always true in our case, so won't change anything.
...
class VoxelNet(nn.Module):
    ...
    def voxel_indexing(self, sparse_features, coords, number_of_occupied_voxels, dinovoxel=None):
        "Densify the sparse voxel features into a dense 3D grid for convolutional middle layers"
    ...
    def forward(self, voxel_features, voxel_coords, number_of_occupied_voxels, dinovoxel=None):
        ...
        if self.use_col:  # if using rpn layers
            ...
            #as in BEVCar with the false
            #used to change the nb of dim from 128 to output_dim if != 128
